...
-- Dzen config
myStatusBar = "dzen2 -x '0' -y '0' -h '16' -w '1585' -ta 'l' -fg '#FFFFFF' -bg '#161616' -fn '-*-bitstream vera sans-medium-r-normal-*-11-*-*-*-*-*-*-*'"
...
myBtmStatusBar = "bash /home/wbrozas/.xmonad/clock.sh | dzen2 -x '1792' -y '0' -h '16' -w '128' -ta 'c' -fg '#FFFFFF' -bg '#161616' -fn '-*-bitstream vera sans-medium-r-normal-*-11-*-*-*-*-*-*-*'"
...
keys' conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm,               xK_p     ), spawn "/usr/bin/xterm")

    -- launch gmrun
    , ((modm, xK_r     ), spawn "/usr/bin/gmrun")

    -- launch browser
    , ((modm, xK_w     ), spawn "/usr/bin/firefox")
    --, ((modm, xK_w     ), spawn "/usr/bin/google-chrome")
